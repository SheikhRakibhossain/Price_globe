$(function () {
	...
	function anchorBlock() {
		document.querySelectorAll('#services a').forEach(function (e) { e.href = 'javascript:;' })
	} anchorBlock();
	...
	window.addEventListener('load', function () {
		setTimeout(() => {
			$(window).ready(function () {
				jQuery('script[src="js/main.js"]').siblings('div').addClass('webHost__monogram')
			})
		}, 0)
		
		setTimeout(() => {
    		document.querySelectorAll('div#features .webHost__monogram').forEach(function (e) {
                const lastChild = e.lastElementChild;
                if (lastChild) {
                    lastChild.style.display = "none !important";
                }
            });
